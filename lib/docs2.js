...
function nestData(pageFileCache, start = '') {
  ...
  for (let page in pageFileCache) {
    ...
    if (fullPath.includes('index.md') && !page.includes('/')) {
      ...
      for (let key in { ...pageFileCache }) {
        if (key.startsWith(page) && key !== page) {
          newCache[key.replace(page + '/', '')] = pageFileCache[key];
        }
      }

      files.push(nestData(newCache, page));
    }
  }
